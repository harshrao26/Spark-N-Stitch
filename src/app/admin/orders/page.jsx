...
export default function AdminOrdersPage({ orders = [] }) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {localOrders.map((order) => (
        <div key={order._id} className="border p-4 rounded-xl shadow bg-white space-y-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-sm gap-2">
            <span className="text-gray-600">User: <strong>{order.userEmail}</strong></span>
          ...
          <div className="grid gap-2">
            {order.items.map((item, i) => (
              <div key={i} className="flex gap-3 items-center border-b pb-2 last:border-none">
                <img
